...
As mentioned, the reduction in sample size from dividing the images based on the bounding box affected the model’s performance. Despite these challenges, the fact that the model successfully located aggregate reticulocytes in images that it had not encountered before proved that with some fine-tuning, this model could be adapted to detect multiple images. Object detection is a field in veterinary medicine that requires further research, particularly in addressing challenges that could contribute to reducing the cost of diagnostic imaging. The necessity for more extensive datasets becomes apparent, as the model would thrive on variance.
